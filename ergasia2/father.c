...
void forward_signal(int sig) {
  ...
  // Αν ο πατέρας λάβει SIGTERM, πρέπει να κλείσει και ο ίδιος αφού ενημερώσει
  // τα παιδιά
  if (sig == SIGTERM) {
    free(child_pids);
    exit(0);
...
void sigchld_handler(int sig) {
  ...
  while ((dead_pid = waitpid(-1, &status, WNOHANG | WUNTRACED | WCONTINUED)) >
         0) {
...
int main(int argc, char *argv[]) {
  ...
  // Για να αποφυγω race conditions, διαβεβαιώνω
  // ότι το sigterm θα παει σε όλα τα παιδιά πριν τερματιστει ο πατέρας

  sigaddset(&sa_forward.sa_mask, SIGCHLD);
